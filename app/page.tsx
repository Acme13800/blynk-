...
export default function BlynkHome() {
  ...
  const [myProfile, setMyProfile] = useState({ displayName: "", username: "", bio: "", email: "", avatarUrl: "", coverUrl: "", presentationVideoUrl: "", city: "" });
  ...
  useEffect(() => {
    ...
    async function loadMyProfile() {
      ...
      if (!user) return;
      const { data } = await client.from("profiles").select("display_name, username, bio, avatar_url, cover_url, presentation_video_url, city, onboarding_completed").eq("id", user.id).maybeSingle();
      if (!data?.onboarding_completed && !data?.bio) { router.replace("/onboarding"); return; }
      setMyProfile({ displayName: data?.display_name || user.user_metadata.display_name || user.email?.split("@")[0] || "Blynk user", username: data?.username || "", bio: data?.bio || "", email: user.email || "", avatarUrl: data?.avatar_url || "", coverUrl: data?.cover_url || "", presentationVideoUrl: data?.presentation_video_url || "", city: data?.city || "" });
