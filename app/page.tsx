...
export default function BlynkHome() {
  ...
  const [myProfile, setMyProfile] = useState({ displayName: "", username: "", bio: "", email: "", avatarUrl: "", presentationVideoUrl: "" });
  ...
  const loadMatches = async () => {
    ...
    if (!data?.length) { setMatchRequests([]); return; }
  ...
  const openConversation = async (request: MatchRequest) => {
    setActiveMatch(request); setTab("messages");
    if (!supabase) return;
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;
    ...
    if (data) setChat(data.map((item) => ({ from: item.sender_id === user.id ? "me" : request.otherName, text: item.content || "" })));
  };
  ...
  useEffect(() => {
    ...
    async function loadMyProfile() {
      ...
      if (!user) return;
      const { data } = await client.from("profiles").select("display_name, username, bio, avatar_url, presentation_video_url").eq("id", user.id).maybeSingle();
      setMyProfile({ displayName: data?.display_name || user.user_metadata.display_name || user.email?.split("@")[0] || "Blynk user", username: data?.username || "", bio: data?.bio || "", email: user.email || "", avatarUrl: data?.avatar_url || "", presentationVideoUrl: data?.presentation_video_url || "" });
  ...
  return <main className="blynk-shell min-h-screen">
    ...
    <div className="mx-auto grid max-w-6xl grid-cols-1 gap-6 px-4 pb-24 pt-6 md:px-6 lg:grid-cols-[185px_minmax(0,1fr)_260px]">
      ...
      <section className="min-w-0">
        {tab === "discover" && <div className="mx-auto max-w-md" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}><div className="mb-4 grid grid-cols-2 rounded-2xl bg-white/5 p-1 text-center text-sm font-bold"><button className="rounded-xl bg-white/10 py-3 text-pink-300">{t.forYou}</button><button className="py-3 text-white/45">{t.nearby}</button></div><article onClick={() => void openPublicProfile(person)} className="blynk-card relative aspect-[9/14] cursor-pointer overflow-hidden rounded-[2rem]" aria-label={`${person.name}, ${person.age}`}><div className={`absolute inset-0 bg-gradient-to-br ${person.accent}`} />{person.video ? <video key={person.id} className="absolute inset-0 size-full object-cover" src={person.video} autoPlay muted loop playsInline /> : person.avatarUrl ? <img src={person.avatarUrl} alt={`Foto de ${person.name}`} className="absolute inset-0 size-full object-cover" /> : <div className="absolute inset-0 grid place-items-center text-7xl">{person.emoji}</div>}<div className="absolute inset-x-0 bottom-0 h-2/5 bg-gradient-to-t from-[#080812]/90 via-[#080812]/35 to-transparent" /><div className="absolute left-4 right-4 top-4 flex items-center justify-between"><span className="rounded-full border border-white/15 bg-black/35 px-3 py-1.5 text-xs font-semibold backdrop-blur">● {person.place}</span><span className="rounded-full bg-black/45 px-3 py-1.5 text-xs backdrop-blur">{person.video ? "▶ Video" : "◉ Foto"} · Ver perfil</span></div><button onClick={(event) => { event.stopPropagation(); notify(language === "es" ? "Perfil guardado" : "Profile saved"); }} className="soft-button absolute right-4 top-16 grid size-11 place-items-center rounded-full border border-white/15 bg-black/35 text-lg backdrop-blur" aria-label="Guardar">♡</button><div className="absolute inset-x-0 bottom-0 p-6"><span className="mb-3 inline-flex rounded-full bg-emerald-400/20 px-3 py-1 text-xs font-bold text-emerald-100">● ONLINE</span><h1 className="text-3xl font-black">{person.name}, {person.age}</h1><p className="mt-2 text-sm leading-6 text-white/85">{person.intro}</p><div className="mt-4 flex flex-wrap gap-2">{person.tags.map((tag) => <span key={tag} className="rounded-full border border-white/15 bg-black/20 px-3 py-1 text-xs font-semibold">{tag}</span>)}</div></div></article><p className="animate-pulse py-3 text-center text-xs text-white/45">↑ {t.swipe}</p><div className="grid grid-cols-[1fr_1.45fr] gap-3"><button onClick={nextPerson} className="soft-button rounded-2xl border border-white/15 bg-white/5 py-4 font-bold text-white/80">× {t.skip}</button><button onClick={() => { void requestMatch(); nextPerson(); }} className="soft-button pink-gradient rounded-2xl py-4 font-bold shadow-[0_12px_28px_#ef38b755]">♥ {t.request}</button></div></div>}
    ...
    {matchRequests.length > 0 && <aside className="fixed bottom-20 right-4 z-40 w-72 rounded-3xl border border-white/15 bg-[#151525f5] p-4 shadow-2xl backdrop-blur-xl sm:bottom-6"><div className="flex items-center justify-between"><p className="text-sm font-black text-pink-300">Matches</p><button onClick={() => void loadMatches()} className="text-xs text-white/50">↻</button></div><div className="mt-3 space-y-2">{matchRequests.filter((request) => request.status !== "rejected").map((request) => <div key={request.id} className="rounded-2xl bg-white/5 p-3"><p className="text-sm font-bold">{request.otherName}</p><p className="mt-1 text-xs text-white/50">{request.status === "pending" ? (request.incoming ? (language === "es" ? "Quiere conectar contigo" : "Wants to connect") : (language === "es" ? "Solicitud enviada" : "Request sent")) : (language === "es" ? "Match confirmado" : "Match confirmed")}</p>{request.incoming && request.status === "pending" && <div className="mt-3 flex gap-2"><button onClick={() => void respondToMatch(request, "rejected")} className="flex-1 rounded-xl bg-white/10 py-2 text-xs font-bold">×</button><button onClick={() => void respondToMatch(request, "accepted")} className="pink-gradient flex-1 rounded-xl py-2 text-xs font-bold">✓</button></div>}{request.status === "accepted" && <button onClick={() => void openConversation(request)} className="mt-3 w-full rounded-xl border border-pink-400/30 py-2 text-xs font-bold text-pink-200">{language === "es" ? "Abrir conversación" : "Open conversation"}</button>}</div>)}</div></aside>}
    {tab === "profile" && <section className="fixed inset-x-0 bottom-0 top-[57px] z-20 overflow-y-auto bg-[#090914] px-4 pb-32 pt-6 lg:px-6"><div className="mx-auto max-w-xl space-y-4"><article className="blynk-card overflow-hidden rounded-[2rem]"><div className="h-32 bg-gradient-to-r from-pink-500 via-fuchsia-600 to-violet-700" /><div className="px-6 pb-6"><div className="-mt-12 flex items-end justify-between"><label className="group relative grid size-24 cursor-pointer place-items-center overflow-hidden rounded-3xl border-4 border-[#151525] bg-gradient-to-br from-amber-300 to-rose-500 text-3xl font-black"><input onChange={uploadAvatar} className="hidden" type="file" accept="image/*" />{myProfile.avatarUrl ? <img src={myProfile.avatarUrl} alt="Foto de perfil" className="size-full object-cover" /> : (myProfile.displayName || "B").slice(0, 1).toUpperCase()}<span className="absolute inset-x-1 bottom-1 rounded-lg bg-black/65 py-1 text-center text-[10px] font-bold opacity-0 transition group-hover:opacity-100">Cambiar</span></label><button onClick={() => setEditingProfile(true)} className="rounded-xl border border-white/15 px-4 py-2 text-sm font-bold">✎ {t.edit}</button></div><h1 className="mt-4 text-3xl font-black">{myProfile.displayName || "Cargando…"}</h1><p className="mt-1 text-sm text-pink-300">@{myProfile.username || "sin_usuario"}</p><p className="mt-1 text-sm text-white/55">{myProfile.email}</p><p className="mt-5 text-sm leading-6 text-white/80">{myProfile.bio || (language === "es" ? "Completa tu presentación para conectar mejor." : "Complete your bio to connect better.")}</p><div className="mt-5 grid grid-cols-3 gap-3 border-t border-white/10 pt-4 text-center text-sm"><span><b>{media.length}</b><br /><small className="text-white/45">contenido</small></span><span><b>{matchRequests.filter((request) => request.status === "accepted").length}</b><br /><small className="text-white/45">matches</small></span><span><b>100%</b><br /><small className="text-white/45">perfil</small></span></div></div></article><article className="blynk-card rounded-3xl p-5"><div className="flex items-center justify-between gap-3"><div><h2 className="font-black">{t.gallery}</h2><p className="mt-1 text-xs text-white/50">Hasta 6 fotos o videos visibles en tu perfil.</p></div><label className="soft-button cursor-pointer rounded-xl bg-white/10 px-3 py-2 text-xs font-bold">+ {t.addMedia}<input onChange={uploadMedia} className="hidden" multiple type="file" accept="image/*,video/*" /></label></div><div className="mt-4 grid grid-cols-3 gap-2">{media.map((source) => <div key={source} className="aspect-square overflow-hidden rounded-xl bg-white/5">{/\.(mp4|webm|mov)(\?|$)/i.test(source) ? <video src={source} className="size-full object-cover" muted playsInline /> : <img src={source} alt="Contenido del perfil" className="size-full object-cover" />}</div>)}{media.length === 0 && <p className="col-span-3 rounded-2xl border border-dashed border-white/15 p-7 text-center text-sm text-white/45">{language === "es" ? "Agrega fotos o videos para que otras personas te conozcan." : "Add photos or videos so others can get to know you."}</p>}</div></article><article className="blynk-card rounded-3xl p-5"><h2 className="font-black">Privacidad y cuenta</h2><p className="mt-2 text-sm text-white/55">Tu nombre, usuario, foto y galería se guardan para esta cuenta.</p><button onClick={() => void signOut()} className="mt-5 text-sm font-bold text-rose-300">↪ {t.logOut}</button></article></div></section>}
